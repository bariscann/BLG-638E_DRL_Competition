...
from inputs.agent import Agent
...
class SelfPlay:
    def __init__(self, team, action_lenght):
        ...
        register_env("ray", lambda config: Agent(args, agents))
        ppo_agent = PPOTrainer(config=config, env="ray")
        ppo_agent.restore(checkpoint_path="/data/inputs/model.tune_metadata") # Modelin Bulunduğu yeri girmeyi unutmayın!
    ...
    def action(self, raw_state):
        ...
        return
        '''
        state = Agent.just_decode_state(raw_state, self.team, self.enemy_team)
        actions, _, _ = self.policy.compute_single_action(state.astype(np.float32))
        location, movement, target, train = Agent.just_take_action(actions, raw_state, self.team)
