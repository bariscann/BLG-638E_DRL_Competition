...
def read_hypers():
    with open(f"./data/config/RiskyValleyNoDesert.yaml", "r") as f:
        hyperparams_dict = yaml.safe_load(f)
        return hyperparams_dict


class NoDesert(BaseLearningAgentGym):
        ...
        for i in range(y_max):
            for j in range(x_max):
                if units[team][i][j]<6 and units[team][i][j] != 0:
                    ...
                    {   
                        'unit': units[team][i][j],
                        'tag': NoDesert.tagToString[units[team][i][j]],
                        'hp': hps[team][i][j],
                        'location': (i,j),
                        'load': load[team][i][j]
                    }
                ...
                if units[enemy_team][i][j]<6 and units[enemy_team][i][j] != 0:
                    ...
                    {   
                        ...
                        'tag': NoDesert.tagToString[units[enemy_team][i][j]],
    ...
    def just_decode_state(obs, team, enemy_team):
        state, _ = NoDesert._decode_state(obs, team, enemy_team)
        return state
